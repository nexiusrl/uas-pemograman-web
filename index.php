...
// Ambil data unit dari database, dikelompokkan berdasarkan nama_unit dan tipe
$query = "SELECT nama_unit, tipe, gambar, deskripsi, MIN(harga) AS harga_mulai,
        COUNT(CASE WHEN status = 'Tersedia' THEN 1 END) AS stok_tersedia
    FROM housing_units
    GROUP BY nama_unit, tipe, gambar, deskripsi
...
<div class="row g-4">
  ...
  $unit["nama_unit"],
    ...
    <p class="small text-muted mb-2">Mulai dari Rp. <?php echo number_format(
      $unit["harga_mulai"],
      0,
      ",",
      ".",
    ); ?></p>
    <span class="badge <?php echo $unit["stok_tersedia"] > 0
      ? "bg-success"
      : "bg-danger"; ?> text-white mb-3" style="font-size: 0.7rem; font-weight: 500;"><?php echo $unit[
   "stok_tersedia"
 ] > 0
   ? "Tersedia: " . htmlspecialchars($unit["stok_tersedia"]) . " Unit"
   : "Habis Terjual"; ?></span>
