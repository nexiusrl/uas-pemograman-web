  ...
  <?php foreach ($units as $unit): ?>
    ...
    <div class="card-body p-4">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <?php echo htmlspecialchars($unit["stok"]); ?> Tersedia
        </span>
      </div>
      <div class="progress" style="height: 6px; background-color: #f1f3f5; border-radius: 10px;">
        <div class="progress-bar <?php echo $bar_color; ?>" role="progressbar" style="width: <?php echo $unit[
  "stok"
] > 0
  ? "100"
  : "0"; ?>%;" aria-valuenow="<?php echo $unit[
  "stok"
]; ?>" aria-valuemin="0" aria-valuemax="100"></div>
      </div>
      <div class="d-flex justify-content-between mt-2" style="font-size: 0.75rem; color: #888;">
        <span>Status</span>
        <span class="fw-semibold text-uppercase" style="font-size: 0.7rem; color: <?php echo $unit[
          "stok"
        ] > 0
          ? "green"
          : "red"; ?>"><?php echo htmlspecialchars($unit["status"]); ?></span>
      </div>
    </div>
    </div>
    </div>
...
<div class="card-body px-4 pb-4">
  ...
  <?php if (empty($units)): ?>
  <tr>
    <td colspan="8" class="text-center py-4 text-muted">Belum ada unit terdaftar.</td>
  </tr>
  <?php else: ?>
  <?php
  $no = 1;
  foreach ($units as $unit): ?>
    ...
    <span class="badge <?php echo $unit["status"] === "Tersedia"
      ? "bg-success"
      : "bg-danger"; ?> bg-opacity-25 <?php echo $unit["status"] === "Tersedia"
   ? "text-success"
