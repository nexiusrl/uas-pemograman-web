...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  if (
    ...
    if ($upload_ok) {
      ...
      $query =
        "UPDATE housing_units SET nama_unit = ?, tipe = ?, harga = ?, stok = ?, status = ?, gambar = ?, deskripsi = ? WHERE id = ?";
      $stmt = mysqli_prepare($conn, $query);
      if ($stmt) {
...
<div class="col-lg-8">
  ...
  <div class="col-md-6 mb-4">
    ...
    <input type="number" class="form-control rounded-3" id="stok" name="stok" value="<?php echo htmlspecialchars(
      $unit["stok"],
    ); ?>" min="0" style="padding: 10px 15px;" />
